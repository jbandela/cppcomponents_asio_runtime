...
#include "external_dependencies/asio/ssl.hpp"

// NB: IGetImplementation returns socket
struct ImplementTlsLowestLevel :implement_runtime_class<ImplementTlsLowestLevel, Tcp_t1>,
  ImplementSocketHelper<ImplementTlsLowestLevel, asio::ip::tcp::socket, asio::ip::tcp>,
  ImplementAsyncStreamHelper<ImplementTlsLowestLevel, asio::ip::tcp::socket>{
  asio::ip::tcp::socket& socket_;
  use<InterfaceUnknown> parent_;
...
asio::ssl::context_base::file_format get_format(std::int32_t format){
  if (format == TlsConstants::Asn){
    return asio::ssl::context_base::asn1;
  }
  else if (format == TlsConstants::Pem){
    return asio::ssl::context_base::pem;
  }
  else{
    throw error_invalid_arg();
  }
...
asio::ssl::verify_mode get_verify(std::int32_t mode){
  if (mode == TlsConstants::VerifyNone){
    return asio::ssl::verify_none;
  }
  else if (mode == TlsConstants::VerifyPeer){
    return asio::ssl::verify_peer;
  }
  else if (mode == TlsConstants::VerifyFailIfNoPeerCert){
    return asio::ssl::verify_fail_if_no_peer_cert;
  }
  else if (mode == TlsConstants::VerifyClientOnce){
    return asio::ssl::verify_client_once;
  }
  else{
    throw error_invalid_arg();
  }
...
asio::ssl::context::method get_method(std::int32_t method){
  if (method == TlsConstants::Method::Sslv2){
    return asio::ssl::context::method::sslv2;
  }
  else if (method == TlsConstants::Method::Sslv23){
    return asio::ssl::context::method::sslv23;
  }
  else if (method == TlsConstants::Method::Sslv23Client){
    return asio::ssl::context::method::sslv23_client;
  }
  else if (method == TlsConstants::Method::Sslv23Server){
    return asio::ssl::context::method::sslv23_server;
  }
  else if (method == TlsConstants::Method::Sslv2Client){
    return asio::ssl::context::method::sslv2_client;
  }
  else if (method == TlsConstants::Method::Sslv2Server){
    return asio::ssl::context::method::sslv2_server;
  }
  else if (method == TlsConstants::Method::Sslv3){
    return asio::ssl::context::method::sslv3;
  }
  else if (method == TlsConstants::Method::sslv3Client ){
    return asio::ssl::context::method::sslv3_client;
  }
  else if (method == TlsConstants::Method::Sslv3Server){
    return asio::ssl::context::method::sslv3_server;
  }
  else if (method == TlsConstants::Method::Tlsv1){
    return asio::ssl::context::method::tlsv1;
  }
  else if (method == TlsConstants::Method::Tlsv11){
    return asio::ssl::context::method::tlsv11 ;
  }
  else if (method == TlsConstants::Method::Tlsv11Client){
    return asio::ssl::context::method::tlsv11_client ;
  }
  else if (method == TlsConstants::Method::Tlsv11Server){
    return asio::ssl::context::method::tlsv11_server;
  }
  else if (method == TlsConstants::Method::Tlsv12){
    return asio::ssl::context::method::tlsv12;
  }
  else if (method == TlsConstants::Method::Tlsv12Client){
    return asio::ssl::context::method::tlsv12_server;
  }
  else if (method == TlsConstants::Method::Tlsv1Client){
...
typedef runtime_class < TlsContextId, object_interfaces<ITlsContext,IGetImplementation>,
  factory_interface < ITlsContextCreator >> TlsContext_t1;

struct ImplementTlsContext :implement_runtime_class<ImplementTlsContext, TlsContext_t1>{
  typedef delegate<std::string(std::size_t len, std::int32_t purpose)> PasswordCallback;
  asio::ssl::context context_;
  void AddVerifyPath(cr_string path){ context_.add_verify_path(path.to_string()); }
  void LoadVerifyFile(cr_string path){ context_.load_verify_file(path.to_string()); }
  void SetDefaultVerifyPaths(){ context_.set_default_verify_paths(); }
  void SetOptions(std::int32_t options){ context_.set_options(options); }
  void SetPasswordCallback(use<PasswordCallback> cb){ context_.set_password_callback(cb); }
  void EnableRfc2818Verification(cr_string host){ context_.set_verify_callback(asio::ssl::rfc2818_verification{ host.to_string() }); }
  void SetVerifyMode(std::int32_t mode){
    context_.set_verify_mode(get_verify(mode));
  }
  void UseCertificateChainFile(cr_string path){ context_.use_certificate_chain_file(path.to_string()); }
  void UseCertificateFile(cr_string path, std::int32_t format){ context_.use_certificate_file(path.to_string(),get_format(format)); }
  void UsePrivateKeyFile(cr_string path, std::int32_t format){ context_.use_private_key_file(path.to_string(),get_format(format)); }
  void UseRsaPrivateKeyFile(cr_string path, std::int32_t format){
    context_.use_rsa_private_key_file(path.to_string(), get_format(format));
  }
  void UseTmpDhFile(cr_string path){ context_.use_tmp_dh_file(path.to_string()); }

  ImplementTlsContext(std::int32_t method) :context_{ get_method(method) }{}
...
CPPCOMPONENTS_REGISTER(ImplementTlsContext)

typedef asio::ssl::stream<asio::ip::tcp::socket> ssl_stream;

ssl_stream::handshake_type get_handshake(std::int32_t type){
  if (type == TlsConstants::Client){
    return ssl_stream::client;
  }
  else if (type == TlsConstants::Server){
    return ssl_stream::server;
  }
  else{
    throw error_invalid_arg();
  }
...
typedef runtime_class < TlsStreamId, object_interfaces<IGetImplementation,ITlsStream>,
  factory_interface<ITlsStreamCreator >> TlsStream_t1;

struct ImplementTlsStream :implement_runtime_class<ImplementTlsStream, TlsStream_t1>,
  ImplementAsyncStreamHelper<ImplementTlsStream,ssl_stream>
{
  ssl_stream socket_;

  ImplementTlsStream(use<ITlsContext> c) :socket_{ get_io(), ::get_implementation<asio::ssl::context>(c) }{}

  Future<void> ITlsStream_Handshake(std::int32_t type){
    
    auto p = make_promise<void>();

    socket_.async_handshake(get_handshake(type), [p](const asio::error_code& error){
      try{
        if (error){
          p.SetError(error.value());
        }
  ...
  Future<void> ITlsStream_Shutdown(){
    auto p = make_promise<void>();
    socket_.async_shutdown([p](const asio::error_code& error){
      try{
        if (error){
          p.SetError(error.value());
        }
  ...
  use<ISocket> ITlsStream_LowestLayer(){
    return ImplementTlsLowestLevel::create(*this).QueryInterface<ISocket>();
